            ...
            <div class="col-lg-4">
                <div class="login-form">
                    <form action="#" method="post" enctype="multipart/form-data">
                        <p>Название товара</p>
                        <input type="text" name="name" placeholder="" value="<?= $product['title']; ?>">
                            ...
                            <?php if (is_array($categoriesList)): ?>
                                <?php foreach ($categoriesList as $category): ?>
                                    <option value="<?php echo $category['id_category']; ?>" 
                                        <?php if ($product['category'] == $category['id_category']) echo ' selected="selected"'; ?>>
                        ...
                        <select name="subcategory_id">
                            <?php if (is_array($subCategoriesList)): ?>
                                <?php foreach ($subCategoriesList as $subCategory): ?>
                                    <option value="<?php echo $subCategory['id_scategories']; ?>" 
                                        <?php if ($product['scategory'] == $subCategory['id_scategories']) echo ' selected="selected"'; ?>>
                                        <?php echo $category['title']; ?>
                                    </option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        ...
                        <textarea name="info"><?php echo $product['info']; ?></textarea>
